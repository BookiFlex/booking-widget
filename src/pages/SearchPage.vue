...
<template>
  <div class="search-page">
    <bflex-search-bar id="searchBar" :check-in-date="dateRange.start" :check-out-date="dateRange.end"></bflex-search-bar>
    <template v-if="loading || loadingAccommodationOffers">
      <Skeleton v-for="i in 3" :key="i"></Skeleton>
    </template>
    <template v-else>
      <InformationBlock v-for="(accommodationOffer) in accommodationOffers" class="accommodation-offer" :key="accommodationOffer.accommodationType.id">
...
.rate-plan-list .rate-plan-card:last-child {
  border-bottom-left-radius: var(--main-border-radius, 10px);
  border-bottom-right-radius: var(--main-border-radius, 10px);
}
